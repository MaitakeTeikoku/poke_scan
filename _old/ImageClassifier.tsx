  ...
  const loadModel = async (modelUrl: string) => {
    try {
      ...
      if (loadedModel) {
        ...
        // 出力層がDenseであることを確認し、そのユニット数を取得
        if (outputLayer.getClassName() === 'Dense') {
          const numClasses = outputLayer.outputShape[1];
          console.log(`モデルのクラス数: ${numClasses}`);
        } else {
          console.log('出力層がDense層ではないため、クラス数が取得できませんでした');
        }
      }
    } catch (error) {
    ...
    if (model && image) {
      ...
      imgElement.onload = async () => {
        try {
          ...
          if (selectedModel === modelOptions[2].url) {
            tensor = tf.browser
              .fromPixels(imgElement)
              .resizeNearestNeighbor([180, 180]) // モデルの入力サイズにリサイズ
              .expandDims(0)
              .toFloat()
